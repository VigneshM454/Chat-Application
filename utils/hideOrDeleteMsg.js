...
async function hideOrDeleteMsg(msgArr,sender,chatId){//here all the msg belong to same chat so smae chatId
    let deleteIdArr=[]
    let hideIdArr=[]
    let deleteImgUrl=[]
    try{
        const chat=await chatModel.findOne({_id:chatId},{participants:1}) 
        const participantLen=chat.participants.length;
